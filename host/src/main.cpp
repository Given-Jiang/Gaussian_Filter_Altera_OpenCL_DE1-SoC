...
//    gettimeofday(&tstart, NULL);//测量代码时间开始
    ...
    std::string binary_file = getBoardBinaryFile("Gaussian_Filter", device);
...
//    cout << "Probe Used Time = " << timeuse << "uS" << endl;
    ...
    kernel = clCreateKernel(program, "Gaussian", &status);
...
void Gaussian(Mat &img, const unsigned int pixels_num, unsigned int rows, unsigned int cols)
{
    size_t GaussianSize = 1;
    cl_int status;
    ...
    cl_event event;
    status = clEnqueueNDRangeKernel(queue, kernel, 1, NULL, &GaussianSize, &GaussianSize, 0, NULL, &event);
...
//    gettimeofday(&tend, NULL);//结束测量
    ...
    Gaussian(in_img, IN_IMG_SIZE, in_img_rows, in_img_cols);
...
//	imwrite("test_cv_gray.jpg", dst);
//    imwrite("test_cv_Gaussian.jpg", dst);
	gettimeofday(&tend, NULL);//结束测量
	timeuse = (unsigned int) (1000000*(tend.tv_sec-tstart.tv_sec)+(tend.tv_usec-tstart.tv_usec)); //换算为微秒
